// leetcode 300. Longest Increasing Subsequence
 ...
 class Solution {
public:
    int find(vector<int>& nums, int index, int previousIndex) {
        if (index >= nums.size()) return 0;


        int include = 0;
        if (previousIndex == -1 || nums[index] > nums[previousIndex]) {
            include = 1 + find(nums, index + 1, index );
        }

        int exclude = find(nums, index + 1, previousIndex );
    ...
    int lengthOfLIS(vector<int>& nums) {
        ...
        int index=0;
        int previousIndex=-1;
        return find(nums, index, previousIndex );
    }
};
