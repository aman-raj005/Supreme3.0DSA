...
? Question: Print Anti-Diagonal (Secondary Diagonal) Elements of a 2D Array
?? Problem Statement:
You are given a 2D array arr of size row x column. Write a function to print only the anti-diagonal (secondary diagonal) elements, i.e., the elements where:

typescript
Copy
Edit
i + j == number of columns - 1
All other elements should be printed as spaces for alignment.
...
?? Input:
A 2D array arr of dimensions row x column (in this function, it’s fixed at 3x3)

Integer values row and column representing its size

?? Output:
Print the secondary diagonal elements of the array

All other positions should print spaces to maintain formatting
...
/*
? Question: Transpose a Square 2D Array In-Place
?? Problem Statement:
You are given a square 2D array (matrix) of size r x c where r == c. Write a function to transpose the matrix in-place, meaning without using any extra array.

A transpose of a matrix is obtained by swapping the element at position (i, j) with the element at position (j, i).
...
?? Input:
A 2D square array arr of size r x c (where r = c = 3)

Integers r and c representing the number of rows and columns

?? Output:
Print the transposed matrix after in-place modification.
